    ...
    public class CartModel : PageModel
    {
        ...
        public IActionResult OnPostCheckout()
        {
            ...
            if (MyCart.Items.Count > 0)
            {
                ...
                int[] cartItemsToRemove = new int[MyCart.Items.Count];
                using (SqlConnection conn = new SqlConnection(_connectionString))
                {
                    conn.Open();
                    ...
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        try
                        {
                            ...
                            using (SqlDataReader reader = cmd.ExecuteReader())
                            {
                                ...
                                while (reader.Read())
                                {
                                    orders[i].StoreId = (int)reader["store_id"];
                                    i++;
                                }
                            }
                            ...
                            cmd.Parameters.AddWithValue("@prod_id", System.Data.SqlDbType.Int);
                            cmd.Parameters.AddWithValue("@qnt", 0);
                            cmd.Parameters.AddWithValue("@price", 0);
                            cmd.Parameters.AddWithValue("@store", System.Data.SqlDbType.Int);
                            cmd.Parameters.AddWithValue("@ord_date", DateTime.Now);
                            cmd.Parameters.AddWithValue("@req_date", DateTime.Now.AddDays(3));

                            cmd.CommandText = "SELECT order_id " +
                                                "FROM orders " +
                                ...
                                for (int i = 0; i < orders.Length && item.Quantity > 0; i++)
                                {
                                    ...
                                    if (availableItems > 0)
                                    {
                                        if (orders[i].Id == 0)
                                        {
                                            ...
                                            //prendo l'utlimo id incrementandolo ma non è corretta, solo per verificare il funzionamento
                                            lastID++;
                                            orders[i].Id =  lastID;
                                            cmd.Parameters["@ord_id"].Value = orders[i].Id;

                                            cmd.CommandText = "SET IDENTITY_INSERT orders ON; " +
                                                "INSERT INTO orders (order_id, customer_id, order_status,order_date, required_date, store_id, staff_id) " +
                                        ...
                                        cartItemsToRemove[iItems] = item.Quantity;
                                    }
                            ...
                            MyCart.Update(cartItemsToRemove); 
